    ...
    async execute(sock, m, msgData) {
        ...
        if (!text) {
            return sock.sendMessage(msgData.remoteJid, {
                text: `Uwaaa! Sayangku mau tanya apa sama Mistral Ryzumi? (˶˃ ᵕ ˂˶)\n\nSilakan masukkan pertanyaan dengan perintah *.mistral <teks>* yaa~! (๑>ᴗ<๑)`
        ...
        try {
            ...
            const modelsToTry = await getTextModels({ brandFilter: 'mistral' });
            let data = null;
            let lastError = null;
            ...
            for (const modelName of modelsToTry) {
                try {
                    ...
                    lastError = err;
                    console.warn(`Mistral model '${modelName}' failed, attempting fallback...`);
                }
            }

            if (!data || !data.result) {
                throw new Error(lastError?.message || data?.message || data?.error || 'Gagal mendapatkan respon dari Mistral AI.. (╥﹏╥)');
            }

            await sock.sendMessage(msgData.remoteJid, { text: cleanAiResponse(data.result) }, getQuoteOption(msgData, m));

        } catch (error) {
            console.error('Mistral AI Plugin Error:', error);
